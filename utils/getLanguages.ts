 ...
 *
 */
export async function getLanguages(doc: Content.AllDocumentTypes, client: Client<Content.AllDocumentTypes>, locales?: string[]) {
    if (locales && locales.length > 1) {
        ...
        return locales.map((lang) => {
            return {
                url: [doc, ...altDocs].find((doc) => lang === doc.lang)?.url || `/${lang}`,
                lang_name: lang,
            };
        })
    }
