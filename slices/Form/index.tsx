        ...
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 lg:grid-cols-12 lg:gap-8 lg:px-8 items-center">
          <div className="max-w-2xl text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl lg:col-span-7">
            <PrismicRichText field={slice.primary.description} components={{
              heading2: ({ children }) => (
                <h2 className="inline sm:block lg:inline xl:block">{children}</h2>
              ),
              paragraph: ({ children }) => (
                <p className="inline sm:block lg:inline xl:block">{children}</p>
              )
            }} />
          </div>
          <form onSubmit={handleSubmit} className="w-full max-w-md lg:col-span-5 lg:pt-2">
            <div className="flex gap-x-4">
              <TextField id="email-address" label="Email address"
                name="email"
                type="email"
                autoComplete="email"
                required
                labelSrOnly
                placeholder={slice.primary.placeholder ? slice.primary.placeholder : undefined}
                className='flex-auto'
              />
            ...
            <PrismicRichText field={slice.primary.disclaimer} components={{
              paragraph: ({ children }) => (
              ...
              hyperlink: ({ children, node }) => (
                <PrismicLink field={node.data} className="font-semibold text-indigo-600 hover:text-indigo-500">
                  {children}
                </PrismicLink>
              )
            }}
