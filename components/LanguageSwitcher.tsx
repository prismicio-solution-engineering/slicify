...
export function LanguageSwitcher(props: { languages: {
    url: string;
    lang_name: string;
}[] }) {
    function handleChange(event: ChangeEvent<HTMLSelectElement>): void {
        //Handle language redirects through the header language switch
        const newVersion = props.languages.find(version => version.lang_name ===  event.target.value )
        if(!newVersion){
            //Redirect to 404 if alternative version does not exist
            router.push('/404')
        }
        else{
            router.push(newVersion.url, newVersion.url, { locale: event.target.value })
        }
    }
