...
export default function LandingPage({ page, header, footer, languages }: LandingPageProps) {
  return (
    <>
      <Head>
...
export async function getStaticProps({ locales, locale, previewData, params }: GetStaticPropsContext<PageParams>) {
  const client = createClient({ previewData });
  //    ^ Automatically contains references to document types

  if (params && params.uid) {
    const page = await client.getByUID<Content.LandingPageDocument>("landing_page", params.uid, { lang: locale });
