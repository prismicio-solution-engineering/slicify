...
export default function BlogIndex({ page, header, footer, languages }: BlogIndexProps) {
  return (
    <>
      <Head>
        <title>{page.data.meta_title || "Slicify - Blog Home"}</title>
        <meta name="description" content={page.data.meta_title || "Slicify Blog, slices for everyone."} />
...
export async function getStaticProps({ locales, locale, previewData }: GetStaticPropsContext) {
  ...
  const page = await client.getSingle<Content.BlogIndexDocument>("blog_index", { lang: locale });
